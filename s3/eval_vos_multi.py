...
from s3.inference.data.vos_test_dataset import VOSTestDataset
from s3.inference.data.burst_test_dataset import BURSTTestDataset
from s3.model.s3 import s3
from s3.inference.inference_core import InferenceCore
from s3.inference.utils.results_utils import ResultSaver, make_zip
from s3.inference.utils.burst_utils import BURSTResultHandler
from s3.inference.utils.args_utils import get_dataset_cfg
...
def eval_vos_worker(gpu, cfg: DictConfig):
     # 初始化环境
    os.environ['MASTER_ADDR'] = 'localhost'
    os.environ['MASTER_PORT'] = '12355'
    torch.distributed.init_process_group('nccl', rank=gpu, world_size=cfg.gpu_num)
    ...
    s3 = s3(cfg).cuda(gpu).eval()
    if cfg.weights is not None:
        model_weights = torch.load(cfg.weights)
        s3.load_weights(model_weights)
    else:
...
#     meta_loader = meta_dataset.get_datasets()
    sampler = DistributedSampler(meta_dataset.get_datasets(), num_replicas=cfg.gpu_num, rank=gpu)
    meta_loader = DataLoader(meta_dataset, batch_size=1, sampler=sampler, shuffle=False, num_workers=4)
...
#         if os.path.exists(os.path.join(mask_output_root, vid_name)):
#             continue

        try:
            processor = InferenceCore(s3, cfg=cfg)
            ...
            for ti, data in enumerate(loader):
                with torch.cuda.amp.autocast(enabled=use_amp):
                    ...
                    prob = processor.step(image, mask, valid_labels, end=(ti == vid_length - 1), vid_name=vid_name)
...
def main(cfg: DictConfig):
    gpu_num = cfg.gpu_num if 'gpu_num' in cfg else torch.cuda.device_count()
    print(gpu_num)
    cfg.gpu_num = gpu_num
    
    # 如果配置了多个 GPU，使用多进程
    if gpu_num > 1:
        mp.spawn(eval_vos_worker, args=(cfg,), nprocs=gpu_num, join=True)
    else:
        eval_vos_worker(0, cfg)
