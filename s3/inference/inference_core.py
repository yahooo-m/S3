...
from s3.inference.memory_manager import MemoryManager
from s3.inference.object_manager import ObjectManager
from s3.inference.image_feature_store import ImageFeatureStore
from s3.model.s3 import s3
from s3.utils.tensor_utils import pad_divide_by, unpad, aggregate
...
def feature_imshow(inp, out_path, title=None):
    """Imshow for Tensor."""
    inp = inp.mean(dim=1)
    inp = inp[0].cpu().detach().numpy() # 将通道数放在最后一维
    inp = np.clip(inp, 0, 1)
    plt.imshow(inp)
...
#         print(self.network)
    ...
    def step(self,
             ...
             *,
             ...
             force_permanent: bool = False, vid_name) -> torch.Tensor:
        """
        ...
        image, self.pad = pad_divide_by(image, 32)
        ...
        out_path = os.path.join('/code/s3/out/s2-demo/d17-480/feature_map',vid_name, str(self.curr_ti+1)+'.png')
        os.makedirs(os.path.join('/code/s3/out/s2-demo/d17-480/feature_map',vid_name), exist_ok=True)
        feature_imshow(ms_feat[1], out_path)
    ...
    def get_aux_outputs(self, image: torch.Tensor) -> Dict[str, torch.Tensor]:
        image, pads = pad_divide_by(image, 32)
        image = image.unsqueeze(0)  # add the batch dimension
