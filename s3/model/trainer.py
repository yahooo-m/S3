...
from s3.model.train_wrapper import s3TrainWrapper
from s3.model.utils.parameter_groups import get_parameter_groups
from s3.model.losses import LossComputer
from s3.utils.log_integrator import Integrator
from s3.utils.image_saver import vis
from s3.utils.logger import TensorboardLogger
from s3.utils.time_estimator import TimeEstimator
...
class Trainer:
    def __init__(self, cfg: DictConfig, stage_cfg: DictConfig, log: TensorboardLogger, run_path):
        ...
        self.s3 = nn.parallel.DistributedDataParallel(s3TrainWrapper(cfg, stage_cfg).cuda(),
        ...
        self.log.log_string('model_size',
                            str(sum([param.nelement() for param in self.s3.parameters()])))
        self.log.log_string(
            'number_of_parameters_that_requires_gradient',
            str(
                sum([
                    param.nelement()
                    for param in filter(lambda p: p.requires_grad, self.s3.parameters())
                ])))
        ...
        self.train()
        parameter_groups = get_parameter_groups(self.s3, stage_cfg, print_log=(local_rank == 0))
    ...
    def do_pass(self, data, it=0):
        ...
        if self.use_amp:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(self.s3.parameters(), self.clip_grad_norm)
...
#         for name, param in self.s3.named_parameters():
...
#                 print(name)
    ...
    def save_weights(self, it, save_copy=False):
        ...
        if save_copy:
            ...
            torch.save(self.s3.module.state_dict(), model_path)
    ...
    def save_checkpoint(self, it, save_copy=False):
        ...
        checkpoint = {
            'it': it,
            'weights': self.s3.module.state_dict(),
    ...
    def load_checkpoint(self, path):
        ...
        map_location = 'cuda:%d' % self.local_rank
        self.s3.module.load_state_dict(weights)
    ...
    def load_weights_in_memory(self, src_dict):
        self.s3.module.load_weights(src_dict)
    ...
    def weights(self):
        return self.s3.module.state_dict()
