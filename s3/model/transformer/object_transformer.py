...
class QueryTransformerBlock(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.read_from_pixel = DQ_CA(self.embed_dim,
                                      self.num_heads,
...
class QueryTransformer(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        #         self.reg_to_query_emb = nn.Linear(768, self.embed_dim)
    ...
    def forward(self,
        ...
        #         reg = self.reg_to_query_emb(reg)
        #         print(reg.size())
        query = self.query_init.weight.unsqueeze(0).expand(bs * num_objects, -1,
                                                           -1) + obj_init  # + reg.unsqueeze(0).expand(num_objects, -1, -1, -1).contiguous().view(bs*num_objects, *reg.shape[-2:])
