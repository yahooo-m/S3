...
from s3.model.modules import *
from s3.model.big_modules import *
from s3.model.aux_modules import AuxComputer
from s3.model.utils.memory_utils import *
from s3.model.transformer.object_transformer import QueryTransformer
from s3.model.transformer.object_summarizer import ObjectSummarizer
from s3.utils.tensor_utils import aggregate
from s3.model.backbones.adapter import D2VitAdapterDinoV2
from s3.model.backbones.dinov2_fpn import DinoV2FPN
from s3.model.mae_adapter.vit_adapter import get_vit_adapter
from s3.model.backbones.adapter import D2VitCOMERDinoV2
...
class s3(nn.Module):
    def __init__(self, cfg: DictConfig, *, single_object=False):
        ...
        #
        if model_cfg.pixel_encoder.diff == 'dinov2':
            self.pixel_encoder = D2VitAdapterDinoV2(model_cfg)
        elif model_cfg.pixel_encoder.diff == 'dinov2comer':
            self.pixel_encoder = D2VitCOMERDinoV2(model_cfg)
        elif model_cfg.pixel_encoder.diff == 'fpn':
            self.pixel_encoder = DinoV2FPN(model_cfg)
        elif model_cfg.pixel_encoder.diff == 'mae':
            self.pixel_encoder = get_vit_adapter(model_cfg)
        else:
    ...
    def encode_image(self, image: torch.Tensor) -> (Iterable[torch.Tensor], torch.Tensor):
        ...
        #print(ms_image_feat[0].size(), ms_image_feat[1].size(),ms_image_feat[2].size(),)
