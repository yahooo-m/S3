...
class InteractionBlockWithCls_Efficient(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        ...
        self.read_from_cls = CrossAttention(dim=dim,
                                            nhead=num_heads) 
    ...
    def forward(self, x, c, cls, blocks, deform_inputs1, deform_inputs2, H, W, finetune=False):
        ...
        c, _ = self.read_from_cls(c, torch.cat([cls, torch.mean(x, dim=1).unsqueeze(1)], dim=1))
...
class InteractionBlock_Efficient(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 drop=0., drop_path=0., with_cffn=True, cffn_ratio=0.25, init_values=0.,
                 deform_ratio=1.0, extra_extractor=False, with_cp=False):
        super().__init__()
    ...
    def forward(self, x, c, blocks, deform_inputs1, deform_inputs2, H, W, finetune=False):
        if finetune:
            for idx, blk in enumerate(blocks):
                x = blk(x)
        else:
...
class DinoV2ViTAdapter(nn.Module):
    def __init__(
        ...
        self.level_embed = nn.Parameter(torch.zeros(2, embed_dim))
...
#                              extra_extractor=(use_extra_extractor),                 
    ...
    def _add_level_embed(self, c2, c3):
    ...
    def forward(self, x):
        ...
        # SPM forward
        c1, c2, c3 = self.spm(x)
        c2, c3 = self._add_level_embed(c2, c3)
        c = torch.cat([c2, c3], dim=1)
...
class D2VitAdapterDinoV2(DinoV2ViTAdapter, Backbone):
    def __init__(self, cfg):
        ...
        finetune = cfg.pixel_encoder.fintune
        finetune_indexes = cfg.pixel_encoder.finetune_index
        ...
        self.strides = list(self._out_feature_strides.values())
        self.num_channels = list(self._out_feature_channels.values())
    def forward(self, x):
        """
        Args:
        ...
        """
        y = super().forward(x)
        #print(y[2].size(), y[1].size(),y[0].size())
#         reg = y[-1]
        return [y[2], y[1], y[0]]
